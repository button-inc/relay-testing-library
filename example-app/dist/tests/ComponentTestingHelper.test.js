...
import compiledTodoListQuery from "./__generated__/ComponentTestingHelperQuery.graphql";
...
const componentTestingHelper = new ComponentTestingHelper({
    component: TodoList,
    testQuery: testQuery,
...
describe("ComponentTestingHelper", () => {
    ...
    it("loads the query", () => {
        ...
        expect(componentTestingHelper.environment.mock.getAllOperations()[0].root.node
            .name).toEqual("ComponentTestingHelperQuery");
    });
    ...
    it("expectMutationToBeCalled function returns a message when the expected mutation is not called", () => {
        ...
        }).toThrow(`Expected mutation testMutation to be called. Mutations called:\n` +
            `CreateTodoMutation`);
    });
    it("expectMutationToBeCalled function returns a message when the expected variables are not returned", () => {
        ...
        }).toThrow(`Expected mutation CreateTodoMutation to be called with:` +
            `
{
  "test": "iwillfail"
}` +
...
//# sourceMappingURL=ComponentTestingHelper.test.js.map